...
object server extends App with RuntimeContext {
  ...
  val idlRouter = "/v2/" -> setupIDLRuntime(
    Set(loginAPI, adminAPI): Set[IRTWrappedService[IO, RequestContext]],
    Set.empty,
    logger
  )
  val legacyRouter = "/v1/" -> prepareHttp4sRouter

  val allRoutes = Router(List(idlRouter, legacyRouter): _*).orNotFound
  ...
  def setupIDLRuntime(
    services: Set[IRTWrappedService[IO, RequestContext]],
    clients: Set[IRTWrappedClient],
    logger: IzLogger
  )(implicit bio: BIORunner[IO], timer: Timer[IO[Throwable, ?]]) = {
    ...
    val authUser: Kleisli[OptionT[IO[Throwable, ?], ?], Request[IO[Throwable, ?]], RequestContext] =
      Kleisli { request: Request[IO[Throwable, ?]] =>
  ...
  private def prepareRequest(request: Request[IO[Throwable, ?]]): Option[RequestContext] = {
    ...
    request.headers
      .find(_.name == "Authorization".ci)
      .map(_.value.split(" ").toList match {
  ...
  private def setupWsContext(
    rt: Http4sRuntime[IO, RequestContext, RequestContext, String, Unit, Unit],
    logger: IzLogger,
    codec: IRTClientMultiplexor[IO]
  ) = {
    ...
    val wsSessionStorage: WsSessionsStorage[IO, String, RequestContext] =
      new WsSessionsStorageImpl[rt.type](rt.self, logger, codec)
